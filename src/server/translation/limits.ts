...
export const BRILLIANT_MAX_TRANSLATION_OUTPUT_BYTES = 56 * 1_024 * 1_024;
...
export const DEFAULT_TRANSLATION_WORKLOAD_LIMITS: TranslationWorkloadLimits =
  Object.freeze({
    ...
    maxOutputBytes: BRILLIANT_MAX_TRANSLATION_OUTPUT_BYTES,
...
export class TranslationOutputAdmission {
  private readonly chunkBytes = new Map<number, number>();
  ...
  constructor(
    ...
    const initialBytes = limits.initialBytes ?? 0;
    assertNonNegativeSafeInteger(initialBytes, "initial output bytes");
    assertNonNegativeSafeInteger(limits.maxOutputBytes, "output byte limit");
    this.totalBytes = initialBytes;
    if (this.totalBytes > limits.maxOutputBytes) {
      throw translationOutputLimitError();
    }
  }

  get admittedBytes(): number {
    return this.totalBytes;
  }

  admitChunk(chunkIndex: number, translatedMarkdown: string): void {
    const nextChunkBytes = Buffer.byteLength(translatedMarkdown, "utf8");
    const previousChunkBytes = this.chunkBytes.get(chunkIndex) ?? 0;
    const nextTotalBytes = this.totalBytes - previousChunkBytes + nextChunkBytes;
    if (nextTotalBytes > this.limits.maxOutputBytes) {
      throw translationOutputLimitError();
    }
    this.chunkBytes.set(chunkIndex, nextChunkBytes);
...
function assertNonNegativeSafeInteger(value: number, label: string): void {
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new TypeError(`Translation ${label} must be a non-negative safe integer.`);
  }
}

function translationOutputLimitError(): TranslationOutputValidationError {
  return new TranslationOutputValidationError(
    "Translation output exceeds the total output byte limit.",
    { retryable: false },
  );
}
