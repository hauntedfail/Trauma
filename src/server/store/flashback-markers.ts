...
export interface ResolvedFlashbackSelection {
  text: string;
  startOffset: number;
  endOffset: number;
}

export interface FlashbackMarkerRange {
  id: string;
...
export class FlashbackMarkerError extends Error {
  constructor(
    message: string,
    public readonly code: "invalid_selection" | "unresolvable_selection" | "invalid_marker_range",
  ) {
    super(message);
    this.name = "FlashbackMarkerError";
  }
}

export function stripFlashbackMarkers(markdown: string): string {
  return replaceOutsideRanges(
    markdown,
    findProtectedMarkdownRanges(markdown),
    (segment) =>
      segment
        .replace(FLASHBACK_MARK_PAIR_PATTERN, "$1")
        .replace(FLASHBACK_OPEN_MARK_PATTERN, ""),
  );
}
...
export function resolveFlashbackSelection(
  markdown: string,
  selection: FlashbackSelectionInput,
): ResolvedFlashbackSelection {
  ...
  const projectedCandidates = findTextCandidates(
    ...
    .filter(
      (
        ...
        resolved: ResolvedFlashbackSelection;
        score: number;
        distance: number;
  ...
  if (candidates.length === 0) {
    throw new FlashbackMarkerError(
...
export function resolveCanonicalFlashbackSelection(
  markdown: string,
  selection: FlashbackSelectionInput,
): ResolvedFlashbackSelection {
  validateSelectionShape(selection);
...
export function applyFlashbackMarkers(
  markdown: string,
  flashbacks: FlashbackMarkerRange[],
): string {
  const { cleanMarkdown, contentHash, projectedMarkdown, protectedRanges } =
    readMarkdownProjection(markdown);
  const sortedFlashbacks = flashbacks
    .flatMap((flashback) => {
      const sourceRange = resolveFlashbackRenderSourceRange({
        contentHash,
        flashback,
        projectedMarkdown,
      });
  ...
  for (const flashback of sortedFlashbacks) {
    validateMarkerRange(flashback, cleanMarkdown.length, cursor, protectedRanges);
    marked += cleanMarkdown.slice(cursor, flashback.startOffset);
    marked += `<mark data-flashback-id="${escapeAttribute(flashback.id)}">`;
    marked += cleanMarkdown.slice(flashback.startOffset, flashback.endOffset);
    marked += "</mark>";
    cursor = flashback.endOffset;
  }
...
export function normalizeFlashbackMarkerRanges(
  markdown: string,
  flashbacks: FlashbackMarkerRange[],
  ...
  return flashbacks.flatMap((flashback) => {
    const normalized = normalizeFlashbackReaderRange({
      contentHash,
      flashback,
      projectedMarkdown,
    });
...
function readMarkdownProjection(markdown: string): MarkdownProjection {
  const cleanMarkdown = normalizeMarkdownLineEndings(stripFlashbackMarkers(markdown));
...
function normalizeFlashbackReaderRange(input: {
  contentHash: string;
  flashback: FlashbackMarkerRange;
  projectedMarkdown: ProjectedMarkdownText;
}): FlashbackMarkerRange | undefined {
  if (input.flashback.contentHash === input.contentHash) {
    if (
      !isValidReaderRange(input.projectedMarkdown, input.flashback) ||
      (
        input.flashback.text !== undefined &&
        input.projectedMarkdown.text.slice(
          input.flashback.startOffset,
          input.flashback.endOffset,
        ) !== input.flashback.text
      )
    ) {
      return undefined;
    }
    ...
    return {
      id: input.flashback.id,
      contentHash: input.flashback.contentHash,
  ...
  if (input.flashback.contentHash !== undefined && input.flashback.contentHash !== null) {
    return undefined;
  }
...
function resolveFlashbackRenderSourceRange(input: {
  contentHash: string;
  flashback: FlashbackMarkerRange;
  projectedMarkdown: ProjectedMarkdownText;
}): MarkdownRange | undefined {
  if (input.flashback.contentHash === input.contentHash) {
    ...
    return mapReaderRangeToSourceRange(input.projectedMarkdown, input.flashback);
  }
  ...
  return normalizeFlashbackMarkerRange(input.projectedMarkdown, input.flashback);
}
...
function validateSelectionShape(selection: FlashbackSelectionInput): void {
...
function resolveProjectedSelection(
  projectedMarkdown: ProjectedMarkdownText,
  selection: FlashbackSelectionInput,
): ResolvedFlashbackSelection | undefined {
  if (
    rangeOverlapsProtectedProjection(
      projectedMarkdown,
      selection.startOffset,
    ...
    throw new FlashbackMarkerError(
      "Selected markdown code cannot be flashbacked",
      "invalid_selection",
    );
  }
...
function validateMarkerRange(
  ...
  if (flashback.id.trim() === "") {
    throw new FlashbackMarkerError(
      "Flashback marker id must be non-empty",
      "invalid_marker_range",
    );
  }

  if (
    !Number.isInteger(flashback.startOffset) ||
    !Number.isInteger(flashback.endOffset) ||
    flashback.startOffset < minimumStartOffset ||
    flashback.endOffset <= flashback.startOffset ||
    flashback.endOffset > markdownLength
  ) {
    throw new FlashbackMarkerError(
      "Flashback marker ranges must be ordered, non-overlapping, and in bounds",
      "invalid_marker_range",
    );
  }
  ...
  if (
    rangeOverlapsProtectedRanges(
      protectedRanges,
      flashback.startOffset,
      flashback.endOffset,
    )
  ) {
    throw new FlashbackMarkerError(
      "Flashback marker ranges cannot overlap markdown code",
      "invalid_marker_range",
    );
  }
}
...
function normalizeFlashbackMarkerRange(
  ...
  const span = findProjectedSpanForSourceRange(projectedMarkdown, flashback);
  if (span === undefined) {
    return undefined;
  }
