...
export async function POST(event: APIEvent): Promise<Response> {
  ...
  try {
    const existing = await connection.repositories.taxonomy.findCategoryByName(
      payload.name,
    );
    if (existing !== undefined) {
      return json({ category: existing }, { status: 200 });
    }
...
async function parseNamePayload(request: Request): Promise<NamePayload> {
