...
describe("toggleMemoryHighlight", () => {
  it("persists highlight toggles to SQLite without mutating CONTENT.md", () => {
      ...
      `
        ...
        try {
          await connection.db.insert(schema.memories).values({
            id: memoryId,
            url: "https://example.com/highlight-toggle",
    ...
    expect(result.created.operation).toBe("highlighted");
...
function resolveBunExecutable(): string {
  ...
  if (executable === undefined) {
    throw new Error("Bun executable is required for highlight toggle tests");
  }
