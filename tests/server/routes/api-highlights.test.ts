...
import {
  ...
  POST,
} from "../../../src/routes/api/highlights";
...
describe("highlights API route", () => {
  ...
  it("rejects highlight writes while backup failsafe is active", async () => {
    ...
    expect(await readFile(join(config.storePath, "memories", memoryId, "CONTENT.md"), "utf8"))
      .not.toContain("<mark data-highlight-id");
  });
});
...
async function makeRoot() {
  const root = await mkdtemp(join(tmpdir(), "trauma-api-highlights-"));
  tempDirs.push(root);
  return root;
}
