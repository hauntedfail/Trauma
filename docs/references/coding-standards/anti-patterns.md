...
- PostgreSQL-style migration metadata definitions such as `SERIAL` in SQLite
  tables that rely on rowid auto-population.
...
- Alternate migration entrypoints that use weaker PRAGMA, hash-validation, or
  atomicity semantics than the primary runtime migration path.
- Default startup paths that import optional migration tooling before knowing
  that the optional migration path is needed.
