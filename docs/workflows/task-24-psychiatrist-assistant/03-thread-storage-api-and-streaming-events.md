...
- If network is denied and Psychiatrist determines that a current web source is
  ...
  with `web_source_permission = "allow_for_this_turn"`, `retry_pair_id`, and
  `retry_turn_id`. The approved retry must target the original `thread_id`,
  `pair_id`, `turn_id`, accepted prompt, memory id, and variant identity so
  permission completes the same pair instead of creating an unrelated pair.
