...
Ownership: this subtask creates and owns `src/server/translation/current-translation.ts`. It implements `resolveCurrentTranslation()` and the core `repairUnavailableTranslation()` helper used by job start. Later reader subtasks consume `resolveCurrentTranslation()` but do not edit this file.
