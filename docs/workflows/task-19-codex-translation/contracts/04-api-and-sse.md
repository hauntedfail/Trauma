...
- `cancel_requested` finalizes to `canceled` when `turn/interrupt` is acknowledged, `turn.completed` reports interrupted status, no in-flight registry ids are available, or `BRILLIANT_CANCEL_GRACE_MS = 30000` expires.
- While a job remains `cancel_requested`, start/reuse returns `409 cancellation_conflict` for the same `(memory_id, lang_code, source_hash)`.
