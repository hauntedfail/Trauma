...
- Codex app-server transport: default Unix socket via `unix://`; loopback WebSocket only as a local development fallback; HTTP is health-probe-only and must not be used for JSON-RPC. `stdio` is out of scope because TRAUMA does not start or supervise the app-server process.
- Codex turn policy: translation turns use `approvalPolicy: "never"`, read-only sandboxing, disabled network access, and prompt-supplied source content only.
