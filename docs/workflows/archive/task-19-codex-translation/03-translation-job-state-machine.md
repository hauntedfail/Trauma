...
- If pending cancellation loses the race to runner claim, reload the job. If it is now `running`, request running cancellation; if it is terminal, return the idempotent or conflict response for that terminal state.
